            ...
            foreach (var item in _items)
            {
                ...
                if (_itemUI != null)
                {
                    if (_itemUI._pickUp)
                    {
                        ...
                        //Đóng UI khi hết items
                        if (_rtItems.Count == 0)
                        {
                            _itemsDropParent.parent.parent.parent.gameObject.SetActive(false);
    ...
    public void displayItems()
    {
        ...
        foreach (var rtItem in _rtItems)
        {
            ...
            if (itemUiController != null)
            {
                ...
                _items.Add(item);
                var _itemIcon = item.transform.Find("Icon").GetComponent<Image>();
